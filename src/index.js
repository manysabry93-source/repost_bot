...
async function getFilterKeywords(env) {
  return getJSON(env, "filter_keywords", []);
}
function textMatchesFilter(text, keywords) {
...
async function pollSources(env) {
  ...
  for (const src of sources) {
    ...
    const fresh = messages
      ...
      .filter((m) => !textMatchesFilter(m.text, filterKeywords));
...
function mainMenu(env) {
  ...
  rows.push([{ text: "🚫 افزودن کلمه فیلتر", callback_data: "add_filter" }]);
...
async function handleAdminMessage(env, msg) {
  ...
  if (state.action === "add_filter") {
    ...
    await tg(env, "sendMessage", {
      chat_id: chatId,
      text: added.length
        ? `✅ به لیست فیلتر اضافه شد: ${added.join("، ")}`
        : "چیزی اضافه نشد.",
    });
    await tg(env, "sendMessage", {
      chat_id: chatId,
...
async function handleCallback(env, cq) {
  ...
  if (data === "add_filter") {
    await setState(env, adminId, { action: "add_filter" });
    await tg(env, "sendMessage", {
      chat_id: chatId,
      text:
        "کلمه یا عبارت(های) فیلتر رو بفرست، هر کدوم در یک خط. اگه متن پست (یا کپشن عکس/ویدیو) شامل هر کدوم از این‌ها باشه، اون پست منتشر نمی‌شه:",
    });
    return;
  }
  if (data === "list_filter") {
    ...
    const buttons = list.map((w, i) => [
      { text: `🗑 ${w}`, callback_data: `rm_filter_${i}` },
    ]);
    await tg(env, "sendMessage", {
      chat_id: chatId,
      text: "برای حذف روی کلمه بزن:",
  ...
  if (data.startsWith("rm_filter_")) {
    ...
    await tg(env, "sendMessage", {
      chat_id: chatId,
      text: removed ? `✅ «${removed}» حذف شد.` : "پیدا نشد.",
    });
    return;
  }
  if (data === "add_rbsrc") {
...
async function processRubikaSourceMessage(env, chatId, body) {
  ...
  // We can detect that a message carried a file (photo/video/etc.) via generic
  // key presence, but re-downloading and re-uploading it needs a Rubika file
  // download endpoint we couldn't verify against live docs — so, like the
  // Telegram-side video case, we forward the text and flag that media was
  // dropped rather than silently losing the whole post.
  const hasMedia = Boolean(
    newMessage.file_inline ||
      extractField(body, "file_inline") ||
      extractField(body, "sticker")
  );

  if (!cleaned && !hasMedia) return; // nothing usable (e.g. a reaction/edit event)

  const filterKeywords = await getFilterKeywords(env);
  if (textMatchesFilter(cleaned, filterKeywords)) return;
  ...
  const msg = {
    text: hasMedia
      ? `${cleaned}\n\n(⚠️ این پیام فایل/عکس داشت که بازنشر خودکارش از روبیکا فعلاً پشتیبانی نمی‌شود)`
      : cleaned,
    photoUrl: null,
    hasVideo: false,
  };

  await deliverMessage(env, msg, signature, telegramTargets, rubikaTargets);
